...
# Function which runs every survival method on either the simulated data 
# or the bootstrap sample
...
runAllMethods<-function(data,d) {
  ...
  ps.mod<-glm(trt~age+as.factor(charlson)+male+low.stage+high.grade+histology+white+hispanic+
  ...
  #Standard weights
  dat$ps.IPTW<-dat$trt*1/dat$ps.pred + (!dat$trt)*1/(1-dat$ps.pred)
  ...
  #Split the dataset at the failure times - too computationally burdensome
  #cut.points <- unique(dat$time[dat$fail == 1])
  
  #Split at 1-month increments
  cut.points<-seq(from=0,to=15,by=1/12)
  ...
  #Too computationally intensive to compute pseudo-obs at each failure
  #Instead use 1-month increments
  cutoffs<-seq(from=1/12,to=15,by=1/12)
  ...
  pseudo.obs.mat.wt<-diag(dat$ps.IPTW)%*%pseudo.obs.mat
  ...
  res.RMS[i,1]<-sum(pseudo.RMS[dat$trt==0]*dat$ps.IPTW[dat$trt==0])/dim(dat)[1]
  res.RMS[i,2]<-sum(pseudo.RMS[dat$trt==1]*dat$ps.IPTW[dat$trt==1])/dim(dat)[1]
  ...
  res.2y[i,1]<-sum(pseudo.obs.df$time.2[dat$trt==0]*dat$ps.IPTW[dat$trt==0])/dim(dat)[1]
  res.2y[i,2]<-sum(pseudo.obs.df$time.2[dat$trt==1]*dat$ps.IPTW[dat$trt==1])/dim(dat)[1]
  
  res.5y[i,1]<-sum(pseudo.obs.df$time.5[dat$trt==0]*dat$ps.IPTW[dat$trt==0])/dim(dat)[1]
  res.5y[i,2]<-sum(pseudo.obs.df$time.5[dat$trt==1]*dat$ps.IPTW[dat$trt==1])/dim(dat)[1]
  
  res.10y[i,1]<-sum(pseudo.obs.df$time.10[dat$trt==0]*dat$ps.IPTW[dat$trt==0])/dim(dat)[1]
  res.10y[i,2]<-sum(pseudo.obs.df$time.10[dat$trt==1]*dat$ps.IPTW[dat$trt==1])/dim(dat)[1]
